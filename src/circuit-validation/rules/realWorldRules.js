...
function hasResistivePathToSupply(validator, startNode) {
    ...
    while (queue.length > 0) {
        ...
        for (const neighbor of neighbors) {
            ...
            const comp = validator.getComponent(neighbor);
            if (validator.isType(comp, 'openhw-resistor', 'resistor')) {
...
export function validateRp2040VoltageInputs(validator) {
    ...
    const PICO_TYPES = ['openhw-raspberry-pi-pico', 'openhw-raspberry-pi-pico-w'];
...
export function validateBuzzerResistor(validator) {
    ...
    validator.components
        ...
        .forEach(buzzer => {
            ...
            // In Openhw buzzer is usually 2 terminals, pin 1 and 2 or + and -
...
export function validateSignalIntegrity(validator) {
    ...
    const hasDigitalDisplay = validator.components.some(c => validator.isType(c, 'openhw-lcd1602-i2c', 'openhw-oled'));
